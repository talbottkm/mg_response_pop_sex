...
# remove birds as appropriate
full <- full %>% filter(primary_dose!="0") %>% filter(secondary_dose!="0")
full <- full %>% filter(sec_inf!="NA") #84
...
# subset secondary infected birds
sinfected <- full %>% filter(sec_inf=="1") # n=22
...
points(jitter(sec_inf, 0.4) ~ jitter(log10(secondary_dose), 0.8), full, pch = 16, 
