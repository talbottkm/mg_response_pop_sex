...
full %>% group_by(year, population) %>% summarize(n=n())
...
#plot(secresist.j)
Anova(secresist.j, secresist.null)
Anova(secresist.j)
confint(secresist.j)
...
# assemble data
prim.infs <- full %>% filter(prim_inf==1)
sec.infs <- full %>% filter(prim_inf==0 & sec_inf==1)

prim.infs <- prim.infs[c(1:3,6,31,33,43)]
sec.infs <- sec.infs[c(1,2,4,6,37,39,44)]
colnames(prim.infs) <- c("band", "sex", "dose", "population",
                         "maxeye", "resistance", "tolerance") 
colnames(sec.infs) <- c("band", "sex", "dose", "population",
                         "maxeye", "resistance", "tolerance") 

prim.infs$inf <- "primary"
sec.infs$inf <- "secondary"

sevdata <- rbind(prim.infs, sec.infs)

load.mod <- lm(resistance~inf+dose, data=sevdata)
summary(load.mod)
#plot(load.mod)

eye.mod <- lm(maxeye~inf+dose, data=sevdata)
summary(eye.mod)
#plot(load.mod)

tol.mod <- lm(tolerance~inf+dose, data=sevdata)
summary(tol.mod)
#plot(tol.mod)


ggplot(sevdata, aes(x = inf, y = resistance, colour = dose)) +
  labs(y= "Resistance", x = "Infection timing")+
  geom_jitter(width=0.1, height=0)

ggplot(sevdata, aes(x = inf, y = maxeye, colour=dose)) +
  labs(y= "Max eye score", x = "Infection timing")+
  geom_jitter(width=0.1, height=0)

ggplot(sevdata, aes(x = inf, y = tolerance, colour=dose)) +
  labs(y= "Tolerance", x = "Infection timing")+
  geom_jitter(width=0.1, height=0)
